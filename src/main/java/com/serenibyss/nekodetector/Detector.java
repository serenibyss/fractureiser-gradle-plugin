...
public final class Detector {
    ...
    public static void scan(final File file, final Logger logger, final boolean errorOnFound) {
        try (JarFile jar = new JarFile(file)) {
            boolean matches = jar.stream()
                    .filter(entry -> entry.getName().endsWith(".class"))
                    .anyMatch(entry -> {
                        try {
                            return scanClass(jar.getInputStream(entry).readAllBytes());
                        } catch (IOException e) {
                            e.printStackTrace();
                            logger.warn("Fractureiser detector failed to scan file: " + file.getPath());
                            return false;
                        }
                    });
            ...
            logger.error("Potential Fractureiser malware found in file: " + file.getPath());
            if (errorOnFound) {
                throw new IllegalStateException("Potential Fractureiser malware found in file: " + file.getPath());
            }
        } catch (IOException e) {
            e.printStackTrace();
            logger.warn("Fractureiser detector failed to scan file: " + file.getPath());
    ...
    private static boolean same(final AbstractInsnNode a, final AbstractInsnNode b) {
    ...
    private static boolean scanClass(final byte[] clazz) {
